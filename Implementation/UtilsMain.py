...
def readPcapOnlyHeaders(pcap_path, output_txt, labels):
    ...
    with open(output_txt, "a", encoding="utf-8") as f:
        ...
        for pkt in PcapReader(pcap_path):
            ...
            header = bytes(tcp)[:tcp.dataofs * 4]
            
            if not header:
                continue

            len_seq.append(len(bytes(tcp)))
    ...
    for folder in os.listdir(folder_path):
        ...
        if not(os.path.isfile(path)):
            for file in os.listdir(path):
                ...
                #readPcapOnlyHeaders(os.path.join(path, file), output_txt, labels)
...
if __name__ == '__main__':
    print("Testing...")
    randomize_bytes('./TrafficData/sni_whs_train.txt')
    #line = os.urandom(400).hex()
    #new = ' '.join([line[i:i+2] for i in range(0, len(line), 2)])
    #print(len(new.split(' ')))
    with open('./TrafficData/sni_whs_random.txt') as f:
        l = f.readline()
        print(l.strip().split('\t')[:-2][-3:])
