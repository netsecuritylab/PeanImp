...
class ModelConfig():
    def __init__(self, config):
        '''
        Basically creates a model with mlm head and one without, loads weights on the mlm one, filters
        out the mlm head, and loads the remaining weights on the bert model, so that we only keep the 
